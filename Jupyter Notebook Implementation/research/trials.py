...
if not env_path.exists() and notebook_root.name == "research":
    env_path = notebook_root.parent / ".env"
...
def create_chroma_collection(store_path: Path, collection_name: str):
    client = PersistentClient(path=str(store_path), settings=Settings(anonymized_telemetry=False))
...
def build_final_system_prompt(description: str, question: str, hits: List[Dict[str, Any]]) -> str:
    ...
    return (
        "You are an expert Palmist. Using the visual analysis below and the retrieved book excerpts, provide a detailed, "
        "respectful, and evidence-based reading in markdown format. Use headings, bullet points, and simple language.\n\n"
        "### User Question\n"
        f"{question}\n\n"
        "### Visual Analysis\n"
        f"{description}\n\n"
        ...
        "Please answer the question clearly and directly, using short sentences and easy words. "
        "If there are multiple points, list them one by one. "
        "Do not invent new lines or mounts beyond what is described above."
    )
